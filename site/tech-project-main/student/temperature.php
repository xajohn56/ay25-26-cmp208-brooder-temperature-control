...
// Student sets a new target temperature. Flask call is kept as a comment until the RPi/API is ready.
if ($_SERVER["REQUEST_METHOD"] === "POST" && $brooder) {
    ...
    if ($target_temp <= 0) {
        ...
        */
        ...
        $stmt = $conn->prepare("INSERT INTO brooder_settings (brooder_id, student_id, target_temp) VALUES (?,?,?)");
        $stmt->bind_param("iid", $brooder["brooder_id"], $current_user_id, $target_temp);

        if ($stmt->execute()) {
            $target = $target_temp;
            $success[] = "Target temperature set to " . $target_temp . " C";
        } else {
            $errors[] = "Could not save temperature.";
        }
    ...
    <div class="cards">
        ...
        <div class="card">
            ...
            <p style="color:var(--teal); font-size:0.85rem; margin:6px 0;">
                ...
                <?php echo $latest ? "Mock reading: " . $latest["recorded_at"] : "Awaiting sensor data from RPi"; ?>
            </p>
        </div>
